...
/// for test
pub fn verify_batch_direct<T,I>(
    transcripts: I,
    signatures: &[Signature],
    public_keys: &[PublicKey],
    deduplicate_public_keys: bool,
...
where
    ...
    I: IntoIterator<Item=T>,
{
    verify_batch_rng_direct(transcripts, signatures, public_keys, deduplicate_public_keys, rand_hack())
}

/// for bos-coster
pub fn verify_batch_bos<T,I>(
    transcripts: I,
    signatures: &[Signature],
    public_keys: &[PublicKey],
    deduplicate_public_keys: bool,
...
/// directly
pub fn verify_batch_rng_direct<T,I,R>(
    transcripts: I,
    signatures: &[Signature],
    public_keys: &[PublicKey],
    deduplicate_public_keys: bool,
    ...
    R: RngCore+CryptoRng,
{
    ...
    verify_batch_equation_test( bs, zs, hrams, signatures, public_keys, deduplicate_public_keys )
}

/// bos-coster
pub fn verify_batch_rng_bos<T,I,R>(
    transcripts: I,
    signatures: &[Signature],
    public_keys: &[PublicKey],
    deduplicate_public_keys: bool,
    ...
    R: RngCore+CryptoRng,
{
    ...
    verify_batch_equation_Bos( bs, zs, hrams, signatures, public_keys, deduplicate_public_keys )
}
...
) -> SignatureResult<()>
{
    ...
    let mut zR:Vec<RistrettoPoint>=Vec::new();
    let mut zA:Vec<RistrettoPoint>=Vec::new();
    for (r,z) in R.zip(zs.iter()){
        zR.push(&r * z);
    }
    for (A,hram) in As.zip(hrams.iter()){
        zA.push(&A.unwrap() * hram);
    }
    let zr =zR.iter().sum::<RistrettoPoint>();
    let za =zA.iter().sum::<RistrettoPoint>();
    let right = (zr+za).compress();
    // We need not return SignatureError::PointDecompressionError because
    // the decompression failures occur for R represent invalid signatures.
    let mut b = false;
