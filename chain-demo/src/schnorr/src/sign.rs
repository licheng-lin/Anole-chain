...
impl PublicKey {
    ...
    /// verify a aggregate signature
    #[allow(non_snake_case,unused_mut,unused_parens)]
    pub fn verify_aggregate<T,I>(&self, mut transcripts: I, s: Scalar, R: &[CompressedRistretto], Rsum: CompressedRistretto)
    -> SignatureResult<()>
    where 
    T: SigningTranscript,
    I: IntoIterator<Item=T>,
   {
       ...
       let hrams: Vec<Scalar> = transcripts.by_ref().zip(0..R.len())
        .map( |(mut t,i)| {
            t.proto_name(b"Schnorr-sig");
            t.commit_point(b"sign:pk",self.as_compressed());
       ...
       let R = RistrettoPoint::vartime_double_scalar_mul_basepoint(&k, &(-A), &s);

       if R.compress() == Rsum { Ok(()) } else { Err(SignatureError::EquationFalse) }
   }
...
/// full node aggregate signatures
pub fn sign_aggregate(signatures: &[Signature])->(Scalar,Vec<CompressedRistretto>,CompressedRistretto)   
 where 
{
